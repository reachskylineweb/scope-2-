...
// 1. GLOBAL EVENT CONFIGURATION (SCOPE 2027)
const EVENT_CONFIG = {
    hospitalName: "GEM Hospitals, India",
    eventName: "SCOPE 2027",
    eventDate: "June 18 - 20, 2027",
    venue: "ITC Grand Chola, Guindy, Chennai",
    phone: "+91 93848 12592",
    whatsapp: "+91 93848 12592",
    email: "Swethasattanathan@gmail.com",
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // 11. Chairman Photo Pop-Forward & Typewriter Letter-by-Letter Animation
    initChairmanTypewriterAndPopAnimation();

    // 12. Automatic Live Countdown Timer to SCOPE 2027 (June 18, 2027)
    initScopeCountdownTimer();

    // 13. Direct Category Pill Automatic Cycle Ticker
    initDirectCategoryPillsAutoTicker();

    // 14. 5 Real-World Questions Pill Automatic Cycle Ticker
    initQuestionsPillsAutoTicker();
});
 ...
 * 11. CHAIRMAN PHOTO POP-FORWARD & TYPEWRITER LETTER-BY-LETTER ANIMATION
 */
function initChairmanTypewriterAndPopAnimation() {
    const section = document.getElementById('welcome-chairman');
    if (!section) return;

    const scopeFrame = section.querySelector('.medical-scope-frame');
    const messageContainer = section.querySelector('.message-body-paragraphs');
    if (!messageContainer) return;

    const paragraphsText = [
        "Dear Colleagues and Friends,",
        "It gives me immense pleasure to welcome you to SCOPE 2027, an international gathering dedicated to the rapidly evolving world of therapeutic endoscopy.",
        "The field of endoscopy has moved far beyond diagnosis. Today, endoscopists are performing increasingly complex interventions that were once considered the domain of surgery or radiology.",
        "SCOPE was created with a simple objective — to bring together expertise, experience and innovation and make advanced endoscopy more accessible to the practising endoscopist.",
        "SCOPE 2027 continues this journey. Through live demonstrations, interactive discussions, hands-on training, challenging cases and interaction with experts from across the world, we hope to create an environment where knowledge is not merely presented but experienced and applied.",
        "I invite you to join us in Chennai for three days of learning, interaction and innovation. Let us continue to push the boundaries of what endoscopy can achieve."
    ];
    ...
    const observer = new IntersectionObserver((entries) => {
        entries.forEach(entry => {
            if (entry.isIntersecting && !hasAnimated) {
                ...
                // Step 1: Trigger Full-Section Zoom Entrance Animation on Dr. Palanivelu's Photo
                if (scopeFrame) {
                    scopeFrame.classList.add('full-cover-pop-anim');
                }

                // Step 2: Start Letter-by-Letter Typewriter Effect after image docks into place (1.8s)
                setTimeout(() => {
                    typewriterEffect(messageContainer, paragraphsText);
                }, 1800);
            }
        });
    }, { threshold: 0.2 });
...
function typewriterEffect(container, paragraphList) {
    container.innerHTML = ''; // Clear existing static text
    
    let currentParaIndex = 0;

    function typeNextParagraph() {
        if (currentParaIndex >= paragraphList.length) return;

        const text = paragraphList[currentParaIndex];
        ...
        function typeChar() {
            if (charIndex < text.length) {
                ...
                if (char === 'S' && text.substring(charIndex, charIndex + 10) === 'SCOPE 2027') {
                    // Bold SCOPE 2027
                    const strong = document.createElement('strong');
                    strong.textContent = 'SCOPE 2027';
                    p.insertBefore(strong, cursorSpan);
                    charIndex += 10;
                } else {
                    const textNode = document.createTextNode(char);
                    p.insertBefore(textNode, cursorSpan);
                    charIndex++;
                }
                setTimeout(typeChar, 14); // 14ms per letter for smooth fast typing
            } else {
                // Paragraph finished typing, remove cursor and start next paragraph
                cursorSpan.remove();
                currentParaIndex++;
 ...
 * 12. AUTOMATIC LIVE COUNTDOWN TIMER TO SCOPE 2027 (June 18, 2027)
 */
function initScopeCountdownTimer() {
    const targetDate = new Date('June 18, 2027 08:30:00').getTime();
 ...
 * 13. DIRECT CATEGORY PILL AUTOMATIC CYCLE TICKER
 */
function initDirectCategoryPillsAutoTicker() {
    ...
    // Auto switch pill every 3.2 seconds
    const interval = setInterval(() => {
        currentIndex = (currentIndex + 1) % pills.length;
        activatePill(currentIndex);
    }, 3200);

    // Click handler to manually choose pill
    pills.forEach((p, idx) => {
 ...
 * 14. 5 REAL-WORLD QUESTIONS PILL AUTOMATIC CYCLE TICKER
 */
function initQuestionsPillsAutoTicker() {
    const qPills = document.querySelectorAll('.q-item-pill');
    if (!qPills || qPills.length === 0) return;

    let currentIndex = 0;

    function activateQPill(index) {
        qPills.forEach((qp, idx) => {
            if (idx === index) {
    ...
    setInterval(() => {
        currentIndex = (currentIndex + 1) % qPills.length;
        activateQPill(currentIndex);
    }, 2800);
}
