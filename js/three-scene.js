...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // Initialize Lightweight Knowledge Meets Endoscope Canvas Animation
    initKnowledgeEndoscopyCanvas();
});
...
/**
 * 5. KNOWLEDGE MEETS ENDOSCOPE ANIMATED CANVAS
 * High-performance 2D Canvas Optic Light Beams & Particles with IntersectionObserver pause (0% impact on site speed)
 */
function initKnowledgeEndoscopyCanvas() {
    const canvas = document.getElementById('knowledge-endo-canvas');
    if (!canvas) return;
    ...
    // Create 35 lightweight optic fiber particles
    const particles = [];
    ...
    for (let i = 0; i < particleCount; i++) {
        particles.push({
            ...
            color: Math.random() > 0.5 ? 'rgba(217, 119, 6, ' : 'rgba(0, 210, 255, ',
    ...
    function render() {
        ...
        // Draw animated optical light gradient beam
        const time = Date.now() * 0.001;
        const grad = ctx.createLinearGradient(0, 0, width, height);
        grad.addColorStop(0, 'rgba(126, 34, 206, 0.04)');
        grad.addColorStop(0.5, 'rgba(217, 119, 6, 0.06)');
        grad.addColorStop(1, 'rgba(0, 210, 255, 0.03)');
        ctx.fillStyle = grad;
        ctx.fillRect(0, 0, width, height);

        // Draw optic fiber particles
        for (let i = 0; i < particleCount; i++) {
            const p = particles[i];
            p.x += p.vx;
            p.y += p.vy;
    ...
    // IntersectionObserver for 0% CPU impact when out of view
